...
#define log_frame(__text, __buffer, __len) \
    HAL_UART_Transmit(_uart, (uint8_t *)__text, strlen(__text), 0xFFFF);               \
    HAL_UART_Transmit(_uart, (uint8_t *)__buffer, __len, 0xFFFF); \
    HAL_UART_Transmit(_uart, (uint8_t *)__lf, 1, 0xFFFF);

void RadioModuleDetector::setLogFrameUART(UART_HandleTypeDef *uart) {
	_uart = uart;
}
...
unsigned int RadioModuleDetector::getBidCosRadioMAC()
{
    return _bidCosRadioMAC;
}

unsigned int RadioModuleDetector::getHmIPRadioMAC()
{
